...
class SummaryWidget(WidgetBase):
    ...
    async def get_data(self, volumes: list[ef_models.Volume]) -> dict:
        def update_dict(tokenDict: dict, resultsDict: dict) -> None:
            if tokenDict:
                for token, count in tokenDict.items():
                    ...
                    resultsDict[lowercaseToken] = (resultsDict[
                                                       lowercaseToken] if lowercaseToken in resultsDict else 0) + count
        ...
        def find_extreme_value(target: dict, extreme: Extremes) -> str:
            sortedDocuments = sorted(list(target.keys()), key=lambda a: target[a],
                                     reverse=(True if extreme == 'max' else False))
        ...
        output_data = {'worksetSize': len(volumes), 'totalWords': total, 'uniqueWords': totalunique,
                       'lengthGraph': document_lengths, 'densityGraph': vocab_density}
